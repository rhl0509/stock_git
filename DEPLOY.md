...
- `RUN_SCHEDULER=true`면 기동 시 인프로세스 스케줄러(APScheduler, KST)가 하나의
  ...
  **agent 15종**: 보고서(일/주/월)·성과추적·이상감지·공시요약·수급·갭리스크·
  사이드카(장중 매분)·실적·리밸런싱·워치독·드리프트·자동손절 (agent/scheduler.py:start).

  > **원본과 동시 운용 시 주의.** 2026-07-29 DB 분리로 원본(`D:\stock_tracker`, 8020,
  > DB `stock_stack`)과 테이블을 더는 공유하지 않아 DB 이중 기록은 없다. 그러나 **알림
  > 채널은 여전히 하나**라 아래 두 가지가 남는다. 원본은 `RUN_SCHEDULER` 기본값이
  > `true`(app.py:284)이고 `.env`에 키가 없어 **켜져 있는 상태가 기본**이다.
  >
  > | 증상 | 원인 |
  > |---|---|
  > | 아침뉴스·공시·가격 알림이 두 번 온다 | 두 스케줄러가 같은 수신자에게 발송 |
  > | **텔레그램 챗봇이 불안정해진다** | 같은 `TELEGRAM_BOT_TOKEN` 으로 `getUpdates` 롱폴링이 둘(양쪽 app.py 의 `start_polling`). 텔레그램은 동시 폴링을 409 Conflict 로 거부하며 두 프로세스가 서로 수신을 뺏는다 |
  >
  > 둘 다 상시 운용하려면 **한쪽 스케줄러를 끄거나**(그쪽 `.env` 에 `RUN_SCHEDULER=false`)
  > 봇 토큰·수신 채널을 분리할 것. 8030 만 쓸 거면 8020 을 끄면 된다.
