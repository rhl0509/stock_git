...
trading/train.py
──────────────────────────────────────────────────────────────────────────────
XGBoost 모델 학습 스크립트 (v1 계보).

■ 상태 (2026-07-17)
  이 내용은 원래 trading/signal.py 라는 이름으로 들어가 있었다(초기 커밋부터
  파일명과 내용이 어긋나 있었음). 내용이 학습 스크립트이므로 train.py 로 이름을
  맞추고, signal.py 에는 호출자가 기대하던 서빙 API(predict 등)를 새로 구현했다.

  단, 이 스크립트가 만드는 v1 아티팩트(model/xgboost_model.json 등)는 현재
  존재하지 않으며 아무도 로드하지 않는다. 운영 모델은 XGBoost_v2/train_v2.py 가
  학습하는 82피처 앙상블이다(trading/feature.py 34피처와는 교집합 16개로 비호환).
  즉 이 파일은 참고용 레거시다 — 재학습은 `python -m XGBoost_v2.train_v2` 를 쓴다.

이 파일은 웹 서버와 별개로 64비트 Python 에서 단독 실행합니다.
학습 완료 후 model/ 디렉토리에 파일을 저장하고
서버에서 POST /ml/reload 를 호출하면 즉시 반영됩니다.
...
def fetch_ohlcv_kiwoom(ticker: str, count: int = OHLCV_COUNT) -> pd.DataFrame | None:
    ...
    try:
        ...
        if not kiwoom.market_data_ready():
            logger.warning("[data] 시세 소스 없음(키움/KIS) → yfinance 폴백")
            return None
